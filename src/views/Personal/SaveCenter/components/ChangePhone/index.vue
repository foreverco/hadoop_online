<template>
  <div class="ChangePhone">
    <p class="ChangePhoneTitle">修改手机号</p>
    <el-form
      :rules="formRules"
      :model="formData"
      ref="phoneForm"
    ...
    >
      <el-form-item label="新手机号码" prop="mobile">
        <el-input v-model="formData.mobile" style="width:50%"></el-input>
      </el-form-item>
...
import { reqchangePhone } from "@/api/userInfo";
export default {
  name: "ChangePhone",
  computed: {
    tokenMsg() {
  ...
  methods: {
    ...
    // 确认修改手机号
    changePhoneFn() {
      let phoneParams = JSON.parse(JSON.stringify(this.formData));
      reqchangePhone(phoneParams)
        .then(res => {
          console.log(res);
          this.logout();
        })
